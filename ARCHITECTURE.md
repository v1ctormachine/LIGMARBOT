...
5. **Panel `TEST (version)`** — calls **`ligmarBot.runUiTestBundle()`**: stops auto-farm if it was running, waits for idle, probes, **`clickChargingSkillCancelUi`** if cancel hint visible, then **`quickCalibrationSession`** (attack the target during the window). Skip calibration or cancel-smoke from console with **`{ runQuickCalibration: false }`** / **`{ fireChargeCancelIfHint: false }`**.
