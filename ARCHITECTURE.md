...
- ✅ **slice 24 (charge skill second tap, v0.3.43)**: If **`isChargingSkillCancelHintVisible()`** matches **`Config.combat.chargingCancelHintSubstrings`** under **`chargingCancelHintScanRoot`**, **`attackUntilProgress`** sends a **second `clickActionBarSlot`** on the **same** opener slot after the first progress wait fails, then waits **`attack progress after charge release tap`**. Toggle **`rankedOpenerSecondTapIfChargingHint`**. Debug: **`ligmarBot.isChargingSkillCancelHintVisible()`**.
...
### Combat: charge skills — second bar tap (slice 24)

- When the first ranked opener wait finds no HP/count progress but the UI shows a visible **“Press to cancel”**-style hint (configurable substrings on **`span.status-description`**), the bot **clicks the same bar slot again** then waits for progress with the **full** **`attackProgressTimeoutMs`**. Tune **`chargingCancelHintSubstrings`** if your locale differs.
