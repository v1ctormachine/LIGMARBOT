...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST (BotVersion.version)`** + **when-to-run** hint (each bullet includes **`[Expected: …]`** notes for console/game checks) → `runUiTestBundle()` (probe, state, planner, skill rank, charge-cancel **probe** + optional **`Config.ui.testButtonFireChargeCancelWhenHintVisible`** / **`fireChargeCancelIfHint`** → `clickChargingSkillCancelUi()`); optional **`Config.ui.testButtonRunQuickCalibration`**. Phase block, stats; **Planner** checkboxes + `ligmarbot.plannerUi.v1`. |
