...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST (BotVersion.version)`**, **Cancel smoke on TEST** + **`ligmarbot.testUi.v1`**, **Opener timing (ms)** + **`ligmarbot.combatUi.v1`**, hint → `runUiTestBundle()`. Phase block, stats; **Planner** + `ligmarbot.plannerUi.v1`. |
