...
- ✅ **Skills DB vs class / bar swap (v0.3.30)**: `saveSkillsToCache` stores **`actionBarFingerprint`** from live `app-action-button` classes + icon URLs; **`loadSkillsFromCache`** (when **`Config.skills.invalidateCacheOnBarMismatch`**) compares to current bar and **removes** stale `localStorage` so a wrong-class cache cannot silently drive the planner after hero change.
...
- Skills: `ligmarBot.clearSkillsCache()` (do this after **class / bar layout** change, then `await ligmarBot.scanSkills()` with auto-farm OFF)
