...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST (BotVersion.version)`** + short **when-to-run** hint under it → `runUiTestBundle()` (logs `probeSelectors`, `readBasicState`, `getAutoFarmStatus`, `summarizePlannerInputs`, `rankAttackSkillsByHeuristic`, charge-cancel hint/target, skills meta; optional **`Config.ui.testButtonRunQuickCalibration`** → `quickCalibrationSession`). Phase block, stats; **Planner (auto-farm)** checkboxes + `ligmarbot.plannerUi.v1` persistence. |
