...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST (BotVersion.version)`** + **4-line** hint (`[Expected]` = short “bot should …” on-screen) → `runUiTestBundle()` (+ optional cancel smoke / calibration flags). Phase block, stats; **Planner** + `ligmarbot.plannerUi.v1`. |
