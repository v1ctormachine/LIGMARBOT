...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST (BotVersion.version)`** → **`runUiTestBundle()`**, **Opener timing (ms)** + **`ligmarbot.combatUi.v1`**, compact hints. Phase block, stats. Boot still **`loadPlannerUiPrefs()`** from **`ligmarbot.plannerUi.v1`** (no panel toggles — set **`ligmarBot.Config.planner.*`** in console). |
...
1. **Planner** — no panel toggles; boot loads **`ligmarbot.plannerUi.v1`** if present (legacy saves from older builds). Otherwise use defaults in **`10-config.js`** or set live: **`ligmarBot.Config.planner.*`**. To persist your own scheme, edit **`localStorage`** or reintroduce a tiny save helper later.
2. From console:  
...
5. **Panel `TEST (version)`** — calls **`ligmarBot.runUiTestBundle()`**: stops auto-farm if it was running, waits for idle, probes, **`clickChargingSkillCancelUi`** if cancel hint visible, then **`quickCalibrationSession`** by default.
   ...
   **When to press it (timing):**
   ...
   | Charge-cancel smoke | **Mid-charge**: cancel hint (e.g. “press to cancel”) **on screen** when the bundle reaches that step. | No hint → step logs “skipped”; normal. |
