...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST (BotVersion.version)`** → **`runUiTestBundle()`** (one-click: stop farm if needed, probes, cancel smoke if hint, **`quickCalibrationSession`**), **Opener timing (ms)** + **`ligmarbot.combatUi.v1`**, compact hint. Phase block, stats; **Planner** + `ligmarbot.plannerUi.v1`. |
