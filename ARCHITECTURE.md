...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST`** → **`runUiTestBundle()`** (stops/restarts auto-farm when it was ON), **Opener timing (ms)** + **`ligmarbot.combatUi.v1`**, **when-to-press** hint. Phase block, stats. Boot **`loadPlannerUiPrefs()`** from **`ligmarbot.plannerUi.v1`** (console **`Config.planner`**). |
