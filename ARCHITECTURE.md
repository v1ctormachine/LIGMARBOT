...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST (BotVersion.version)`** + **4-line** hint (**`[Ожидается] …` Russian** on-screen expectations) → `runUiTestBundle()` (+ optional cancel smoke / calibration). Phase block, stats; **Planner** + `ligmarbot.plannerUi.v1`. |
...
- ✅ **slice 24d (v0.3.46)**: TEST panel hint lines (**`Когда:`** / **`[Ожидается]`** / дым отмены / калибровка) **localized to Russian**; **`ROADMAP.md`** **Current focus** = soak-test auto-farm.
