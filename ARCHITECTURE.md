...
- **Console collector (skill tree → master export):** **`bot/tools/skill-master-collector-console.js`** is **not** a Tampermonkey script — paste it only into **DevTools → Console** on **`ligmar.io/game/`** (with the bot loaded so **`ligmarBot.parseSkillEffects`** / **`Config.selectors`** match **`82-skills.js`**). After paste you get **`lmcOne`**, **`lmcAll`**, **`lmcDownload`** on **`window`**:
  - **One class:** Open **Character → Skills** (or let the script open them). Run `await lmcOne("assassin")` (string = class label you are on). JSON is **`console.log`**’d and returned. Popup detection prefers **`app-modal`** whose **`.modal-header-content`** matches **“Skill”**, then **`.dialog-action app-action-info`** (character-sheet layout; not always under **`.cdk-overlay-container`**). Close uses that modal’s **`.modal-header-close`**. Parsing falls back to selectors relative to the **`app-action-info`** root (Config selectors often start with **`app-action-info …`**, which does not match when the query root is already that element).
