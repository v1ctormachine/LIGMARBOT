...
| `90-ui.js`             | In-page panel: ON/OFF, **`TEST (BotVersion.version)`**, **Cancel smoke on TEST** + **Calib on TEST** + **`ligmarbot.testUi.v1`**, **Opener timing (ms)** + **`ligmarbot.combatUi.v1`**, hint → `runUiTestBundle()`. Phase block, stats; **Planner** + `ligmarbot.plannerUi.v1`. |
...
- ✅ **slice 24c (v0.3.45)**: **`runUiTestBundle`** / **`TEST (version)`** panel button, compact **`[Expected]`** hint, **`Config.ui.testButtonFireChargeCancelWhenHintVisible`** + **`fireChargeCancelIfHint`** smoke cancel, **`dispatchClickAt`**, **`getChargeCancelMapGapClientPoint`** on **`ligmarBot`**, **`ROADMAP.md`**, ship rule: **bump `build.ps1 -Description` on every module change** so refresh-only updates work.
- ✅ **slice 24d (v0.3.46)**: Panel checkbox **Cancel smoke on TEST** → **`Config.ui.testButtonFireChargeCancelWhenHintVisible`**; persisted **`localStorage[ligmarbot.testUi.v1]`**; **`loadTestUiPrefs` / `saveTestUiPrefs`** (`90-ui.js`); **`Runtime.ui.testCancelSmokeCheck`**.
...
- ✅ **slice 27 (v0.3.49)**: Panel checkbox **Calib on TEST** → **`Config.ui.testButtonRunQuickCalibration`**; persisted with cancel-smoke in **`ligmarbot.testUi.v1`**; **`Runtime.ui.testRunCalibCheck`**; **`runUiTestBundle`** warns via **`Logger.warn`** if calibration runs while **`getAutoFarmStatus().running`** (`90-ui.js`).
...
- **`Config.combat.rankedOpenerChargeGraceMs`** (default **0**) — after **`postRankedSkillClickSettleMs`**, extra **`sleep`** before the first progress wait on **ranked opener only**; use when the client needs a beat before HP updates (e.g. **200–400** ms trials).
