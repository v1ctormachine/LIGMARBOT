...
- **Ranked opener (Pack A)**: when **`Config.planner.useRankedAttackSkillsInCombat`** is on, **`plannerPickSkillOpeningPick()`** updates **`Runtime.planner`** on every attempt (`lastOpeningPickReason`, `lastOpeningPickDetail`, `lastOpeningPickAt`). Empty cache, no attack skills in the ranker, or every candidate skipped (MP floor, live cooldown hints, **`excludeSlots`**, no direct-damage effects for opener) yields **`all_candidates_filtered`** with a per-cause **breakdown**; throttled **`[PLANNER]`** logs explain why combat fell back to basic. Console: **`ligmarBot.getPlannerOpeningPickDiagnostics()`**.
...
| `20-runtime.js`        | `Runtime` (mutable state: autoFarm, exploration, zoom flag, UI refs). **Pack A:** **`Runtime.planner`** holds the last ranked-opener pick outcome (`lastOpeningPickReason` / `lastOpeningPickDetail` / `lastOpeningPickAt`) plus log-throttle keys for `[PLANNER]` messages when no slot is eligible. |
| `30-utils.js`          | Pure helpers: `isGamePage`, `getCssPath`, `parseFractionText`, `parseFirstInt`, `sleep`, `toShortJson`, `isElementVisible`, `clickElementSafe`, `dispatchMouseAt`, **`dispatchClickAt`**, `parseCoordsText`. |
...
| `86-planner.js`        | **Phase C4 -- paper + calibration + heuristics.** `calibrateEnemyFromCombat`, `rankAttackSkillsByHeuristic`, `plannerSkillEffectHeuristicScore`, **`plannerPickSkillOpeningPick`** (sets **`Runtime.planner`** every call; reasons include `ranked_disabled`, `empty_cache`, `no_attack_skills_for_ranker`, `picked`, `all_candidates_filtered` with MP/CD/exclude breakdown), **`plannerMaybeLogOpeningPickFailure`** (throttled console hint; **`Config.planner.logOpeningPickFailures`** / **`openingPickFailureLogThrottleMs`**), **`getPlannerOpeningPickDiagnostics`**, **`plannerOpenerHoldCastMs`** (slice 12), **`plannerPickSkillSlotToCast`**, slice 11 cooldown gate via **`isActionBarSlotShowingCooldown`**. |
...
| `99-bootstrap.js`      | `start()`, `window.ligmarBot` debug API (prefs: **`saveAllUiPrefs` / `loadAllUiPrefs`** + per-key helpers), **`getPlannerOpeningPickDiagnostics`**, boot warn if ranked combat on with empty skill cache, IIFE close, `start()` invocation. |
...
- ✅ **TEST self-service (v0.3.60+)**: Panel **TEST** drives **`runUiTestBundle`** end-to-end so you do not hand-run playbook console steps: auto **`scanSkills()`** when the cache is empty / bar has no non-empty slots or when **`forceSkillScan: true`**; **`readHeroCombatStats()`** (soft check); ranked-opener **dry-run** via **`plannerPickSkillOpeningPick({})`** + **`getPlannerOpeningPickDiagnostics()`** (critical when ranked combat is on and reason is **`empty_cache`** / **`no_attack_skills_for_ranker`**); then probes, optional cancel smoke, **`quickCalibrationSession`** (calibration failure is **critical** only if **`strictCalibration: true`**). **`Logger.log("TEST","SUMMARY",…)`** plus **`console.table`** over **`checks`**; return object includes **`ok`**, **`criticalOk`**, **`checks`**, **`softFail`**. Under the button, **`Runtime.ui.testResultLine`** shows **`Test result: successful`** (green) when **`res.ok`**, else **`Test result: failed`** (red). Opt out: **`runSkillScanIfNeeded: false`**, **`runHeroStatsInTest: false`**, **`runQuickCalibration: false`**, etc. (`90-ui.js`, `20-runtime.js`).
