...
| `90-ui.js`             | In-page panel: ON/OFF, phase block, stats; **Planner (auto-farm)** checkboxes (`recordEnemyDbBeforeAttack`, `logPlannerAfterSecureTile`, `useRankedAttackSkillsInCombat`) + `ligmarbot.plannerUi.v1` persistence. |
...
1. **Control panel** — section **“Planner (auto-farm)”**: checkboxes mirror **`Config.planner`**. Choices persist in **`localStorage[ligmarbot.plannerUi.v1]`** across reloads.
