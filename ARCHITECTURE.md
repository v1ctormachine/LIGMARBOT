...
| `99-bootstrap.js`      | `start()`, `window.ligmarBot` debug API (includes **`loadPlannerUiPrefs` / `savePlannerUiPrefs`**, **`loadCombatUiPrefs` / `saveCombatUiPrefs`**), boot warn if ranked combat on with empty skill cache, IIFE close, `start()` invocation. |
