...
| `99-bootstrap.js`      | `start()`, `window.ligmarBot` debug API (prefs: **`saveAllUiPrefs` / `loadAllUiPrefs`** + per-key helpers), boot warn if ranked combat on with empty skill cache, IIFE close, `start()` invocation. |
