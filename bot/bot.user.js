...
(function () {
  ...
  function pickAutoChatSpammerMessage() {
    const messages = Config.chat && Array.isArray(Config.chat.messages) ? Config.chat.messages.filter(Boolean) : [];
  ...
  async function maybeRunAutoChatSpammer(liveState, userOpts) {
    ...
    const messages = Array.isArray(Config.chat.messages) ? Config.chat.messages.filter(Boolean) : [];
    if (messages.length <= 0) {
  ...
  async function runUiTestBundle(userOpts) {
    ...
    try {
      ...
      try {
        const chatMessages = Config.chat && Array.isArray(Config.chat.messages) ? Config.chat.messages.filter(Boolean) : [];
        const probeMessage = chatMessages.length > 0 ? chatMessages[0] : "";
