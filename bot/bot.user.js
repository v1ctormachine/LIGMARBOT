...
// @version      1.2.9-beta
// @description  Public beta retarget queue lock and basement entry suppression
...
const BotVersion = {
  version: "1.2.9-beta",
  description: "Public beta retarget queue lock and basement entry suppression",
  builtAt: "2026-06-22T15:52:09.996Z"
};
  ...
  async function tryUseRecoverySkill(resource) {
    ...
    for (let i = 0; i < slots.length; i++) {
      const s = slots[i];
      if (!s || s.kind !== "skill" || !Array.isArray(s.effects)) continue;
  ...
  function classifySupportBuff(skill) {
    ...
    // Exclude safety barriers (such as Windy Dome) from automated pipelines
  ...
  function plannerPickSkillOpeningPick(userOpts) {
    ...
    for (let i = 0; i < slots.length; i += 1) {
      const row = slots[i];
      if (!row || row.kind !== "skill" || exclude.has(i)) {
        continue;
      }
