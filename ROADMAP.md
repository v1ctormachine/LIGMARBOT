...
- **Out-of-combat heal gate rollback (v0.3.145 re-ship)** — **v0.3.146–v0.3.147** added (then narrowed) an HP wait before idle **`exploreByScan`** tile moves; that gate is **removed again** so empty-tile exploration is immediate. **Parsed potion sustain and mana protection** from the original **v0.3.145** line remain unchanged.
...
2. **Current-build field validation** — run long unattended farming with **`TEST`** + normal ON sessions on the main build, then tune only from real failures: execute false-positives, potion waste, or ranked fallback/no-progress spikes.
3. **Planner v2 / runtime sequencing** — the combat-readiness pack is now in; the next real combat step is no longer “better charge %”, but whether the runtime should grow beyond one follow-up horizon into a true queued multi-skill rotation engine.
...
5. **Future: two-skill queue** — Planner still does not model B-while-A (`ARCHITECTURE.md`), so only start this after `v0.3.143` proves stable on the current build.

## Parking lot

- **Chat autospammer (promocode lines)** — optional panel feature to send a fixed chat message on a safe interval (DOM selectors for input + send, backoff on errors, hard rate cap). **Opt-in only**; document ToS / moderation risk before shipping; useful for repeating official-style promo text when the game allows it.
